...
// Since we're running in Node.js, we'll simulate the function logic
function stripEmojis(text) {
    ...
    // Clean up multiple spaces left behind and trim
    return clean.replace(/\s+/g, ' ').trim();
}

console.log('=== Testing stripEmojis() Function ===\n');
...
// Additional test: Verify Unicode property escapes work
console.log('=== Testing Unicode Property Support ===');
try {
    // Test if the regex works with Unicode property escapes
    const testStr = 'Test 🎉';
    const regex = /(\p{Emoji_Presentation}|\p{Extended_Pictographic})/gu;
    const hasUnicodeSupport = regex.test(testStr);
    console.log(`Unicode property escape support: ${hasUnicodeSupport ? '✓ Available' : '✗ Not available'}`);

    if (!hasUnicodeSupport) {
        console.log('Note: Unicode property escapes may not be supported in this Node.js version.');
        console.log('The regex will need a fallback for older environments.');
    }
} catch (error) {
    console.log(`Unicode property escape error: ${error.message}`);
}
...
console.log('5. ✓ Legitimate script characters are preserved (Devanagari, CJK, Cyrillic, Arabic, Latin, Greek)');
console.log('6. ✓ Multiple spaces are normalized');
console.log('7. ✓ Edge cases handled (null, empty string)');
